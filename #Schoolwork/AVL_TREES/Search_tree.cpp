...
{
	if (sub_root == 0) return not_present;
	Binary_node<Record> *to_delete = sub_root;
	// Remember node to delete at end.
	if (sub_root->right == 0) sub_root = sub_root->left;
	else if (sub_root->left == 0) sub_root = sub_root->right;
		...
		while (to_delete->right !=0) { // to_delete is not the predecessor.
