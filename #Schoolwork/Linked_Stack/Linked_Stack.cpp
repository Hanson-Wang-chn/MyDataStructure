...
Error_code MyLinkedStack::push(const Node_entry &item)
// Pre: None
...
Error_code MyLinkedStack::top(Node_entry &item) const
...
{
    Node * new_top = nullptr, * new_copy = nullptr;
    Node * original_node = original.top_node;
    if (original_node == nullptr) {
        new_top = nullptr;
    } else {
